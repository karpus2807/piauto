...
def draw_storage_icon(oled, kind, x, y, fill=1):
    ...
    draw_bitmap_scaled(oled, icon, x, y, len(icon[0]), len(icon), fill=fill)
...
def draw_builtin_icon(oled, name, x, y, w=14, h=14, fill=1):
    ...
    # Prefer 1:1 blit for native 14x14 inside <=16 boxes (centered).
    if int(w) <= 16 and int(h) <= 16:
        native_w = len(bitmap[0])
        native_h = len(bitmap)
        dx = max(0, (int(w) - native_w) // 2)
        dy = max(0, (int(h) - native_h) // 2)
        draw_bitmap_scaled(oled, bitmap, x + dx, y + dy, native_w, native_h, fill=fill)
    else:
